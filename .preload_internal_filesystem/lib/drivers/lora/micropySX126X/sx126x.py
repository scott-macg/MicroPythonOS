...
class SX126X:
    ...
    def receive(self, data, len_, timeout_en, timeout_ms):
        ...
        if modem == SX126X_PACKET_TYPE_LORA:
            ...
            try:
                ...
                print("receive got exception because SX126X_PACKET_TYPE_GFSK detected")
                timeout = 16
                #return ERR_UNKNOWN
        else:
            return ERR_UNKNOWN
    ...
    def setTCXO(self, voltage, delay=5000):
        ...
        if abs(voltage - 0.0) <= 0.001:
            print("setTCXO <= 0.001 so doing self.reset()")
            return self.reset()
